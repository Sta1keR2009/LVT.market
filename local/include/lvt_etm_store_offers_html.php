...
function lvt_etm_store_offers_desktop_html(
    ...
    $priceHeaderHtml = lvt_getchips_price_header_html($storeUsdToRub, $rateDateLabel);
        ...
        <table class="getchips-offers__table js-getchips-offers-table store-offers-table"
            ...
            <?php foreach ($storeData as $store): ?>
                ...
                foreach ($basePrices as $idx => $price) {
                    ...
                    $priceHtmlLines[] = '<div class="getchips-offers__price-tier js-getchips-price-tier" data-tier-qty="' . $tierQty . '" data-price-rub="' . htmlspecialcharsbx((string) $tierPriceRub) . '">х ' . $tierQty . ' шт. ' . number_format($tierPriceRub, 2, '.', '') . ' ₽;</div>';
                }
