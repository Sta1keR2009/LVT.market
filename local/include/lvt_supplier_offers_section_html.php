...
function lvt_supplier_offers_section_html(array $arResult, array $options = []): string
{
    ...
    $priceHeaderHtml = lvt_getchips_price_header_html($usdToRub, $rateDate !== '' ? $rateDate : date('d.m.Y'));
                ...
                foreach ($storeData as $store) {
                    ...
                    if ($basePrices) {
                        ...
                        foreach ($basePrices as $p) {
                            ...
                            $line = 'х ' . $qtyTier . ' шт. ' . $priceFormatted . ' ₽;';
            ...
            if ($promRows !== []) {
                ...
                foreach ($promRows as $pr) {
                    ...
                    if ($tiersRub !== []) {
                        ...
                        $currencyLabel = $sourceCurrency === 'RUB' ? '₽' : $sourceCurrency;
                        $lines = [];
    ...
    $body = str_replace('class="getchips-offers__table js-getchips-offers-table"', 'class="getchips-offers__table js-getchips-offers-table" data-usd-to-rub="' . htmlspecialchars((string)$usdToRub, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '" data-display-currency="RUB"', $body);
    ...
    return '<section class="lvt-supplier-offers" id="lvt-supplier-offers">'
        . '<h2 class="lvt-supplier-offers__title">Наличие по складам</h2>'
