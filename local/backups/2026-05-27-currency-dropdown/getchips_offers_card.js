(function () {
    ...
    function formatPriceTierLine(qty, priceRub, currency, rate, tierEl) {
        ...
        return 'х ' + q + ' шт. ' + formatCurrencyValue(rub, 'RUB') + ' руб.;';
    }
    ...
    function applyCbrRateToTable(table, rate, rateDate) {
        ...
        var rubAlert = table.querySelector('.js-getchips-rub-alert');
        if (rubAlert && rateDate) {
            rubAlert.setAttribute('data-notice', 'Цена в рублях ориентировочная: если курс ЦБ изменится более чем на 2%, итоговую сумму уточним по актуальному курсу.');
        }
        applyCurrencyToTable(table, table.getAttribute('data-display-currency') || 'USD');
    }
    ...
    function ensureModal() {
        ...
        el.innerHTML = ''
            ...
            + '<span class="getchips-modal__title">Предложения поставщиков</span>'
