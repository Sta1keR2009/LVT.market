if (typeof window.JLogo  === 'undefined') {
    ...
    window.JLogo.prototype = {
        ...
        get srcDark() {
            return (typeof arAsproOptions !== 'undefined' && arAsproOptions.THEME && arAsproOptions.THEME.LOGO_IMAGE) ? arAsproOptions.THEME.LOGO_IMAGE : '';
        },

        get srcLight() {
            if (typeof arAsproOptions !== 'undefined' && arAsproOptions.THEME) {
                return arAsproOptions.THEME.LOGO_IMAGE_WHITE || arAsproOptions.THEME.LOGO_IMAGE || '';
            }
            return '';
        },

        get srcFallback() {
            return this.srcDark || this.srcLight || this.src || '';
        },

        ensureValidSrc: function(preferColor) {
            if (!this.img) {
                return;
            }
            ...
            var color = (preferColor === 'light' || preferColor === 'dark')
                ? preferColor
                : this.getInvertedColor(this.themeColor);
            var colorSrc = (color === 'light' ? this.srcLight : this.srcDark);
            var fallbackSrc = colorSrc || this.srcFallback;
        ...
        set color(value) {
            if (this.canChangeColor) {
                if (this.color !== value) {
                    var newSrc = (value === 'light' ? this.srcLight : this.srcDark);
                    if (newSrc) {
                        this.src = newSrc;
                    }
                }
        ...
        setColorOfBanner: function (banner) {
            if (
                ...
                if (typeof window.headerLogo !== 'undefined') {
                    var value = BX.data(banner, 'color');
        ...
        init: function() {
            if (!this.inited) {
                ...
                if (this.img) {
                    if (!this.src) {
                        const fallbackSrc = this.srcFallback;
                        if (fallbackSrc) {
                            this.src = fallbackSrc;
                        }
                    }

                    var newColor = this.getInvertedColor(this.themeColor);
                    var newSrc = (newColor === 'light' ? this.srcLight : this.srcDark);
                    if (newSrc) {
                        this.color = newColor;
                    } else if (!this.src) {
                        const fallbackSrc = this.srcFallback;
        ...
        bindEvents: function() {
            ...
            // Some third-party scripts can blank image src after init.
            // Restore logo source immediately if it happens.
            if (this.img && typeof MutationObserver !== 'undefined') {
