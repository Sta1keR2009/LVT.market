			...
			?>
				<td class="basket-items-list-item-picture" {{#HAS_SERVICES}}rowspan="2" {{/HAS_SERVICES}}>
					<div class="basket-item-block-image<?= (!isset($mobileColumns['PREVIEW_PICTURE']) ? ' hidden-xs' : '') ?>">
						{{#USE_FAST_VIEW}}
							<div class="fast_view rounded-x" data-event="jqm" data-param-form_id="fast_view" data-param-iblock_id="{{IBLOCK_ID}}" data-param-id="{{PRODUCT_ID}}" data-param-item_href="{{DETAIL_PAGE_URL}}" data-name="fast_view">
						...
						<img class="basket-item-image js-popup-image rounded-x" alt="{{NAME}}" src="{{{IMAGE_URL}}}{{^IMAGE_URL}}<?= $templateFolder ?>/images/no_photo.png{{/IMAGE_URL}}">
