<?
	...
	{
		//usort($arResult["GRID"]["ROWS"], 'TSolution::cmpByID');
		$arImages = array();
		foreach($arResult["GRID"]["ROWS"] as $key=>$arItem)
		{
			...
			if(strpos($arItem["PRODUCT_XML_ID"], "#")!==false){
				...
				$arItem1 =TSolution\Cache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => TSolution\Cache::GetIBlockCacheTag($arItem["IBLOCK_ID"]))), array("IBLOCK_ID" => $arItem["IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "XML_ID" => $arXmlID[0]), false, false, array("ID", "IBLOCK_ID"));
            ...
            if ($arItem["PRICE"] == '0') {
                $value = match(TSolution::GetFrontParametrValue('MISSING_GOODS_PRICE_DISPLAY')) {
                    ...
                    'NOTHING' => '',
                    default => $arItem['PRICE_FORMATED'],
                };
                $arResult["GRID"]["ROWS"][$key]["PRICE_FORMATED"] = $value;
            }
		...
		//usort($arResult["GRID"]["HEADERS"], 'TSolution::cmpBySort');
