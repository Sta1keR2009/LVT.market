...
<?if($arResult["NavPageCount"] > 1):?>
	...
	{
		$pattern = strpos($arResult["NavQueryString"], '&amp;') !== false ? '&amp;' : '&';
		$arUrl = explode($pattern, $arResult["NavQueryString"]);
		if($arUrl)
		{
				...
				<?if(!$bPrevDisabled):?>
					...
					<a href="<?=$arResult["sUrlPath"]?><?=$url?>" class="arrows-pagination__prev arrows-pagination__item rounded-x" title="<?=GetMessage("nav_prev")?>">
						<span class="arrow-all arrow-all--reverce arrow-all--wide arrow-all--sm stroke-theme-target">
							<?//=TSolution::showSpriteIconSvg(' arrow-all__item-arrow', SITE_TEMPLATE_PATH.'/images/svg/Arrow_map.svg');?>
