                ...
                if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
                {
                    ...
                    <div class="blog-comment-content">
                        ...
                        if(strlen($comment["urlToHide"])>0)
                        {
                            ...
                            <span class="blog-comment-show">
                                <?if($arParams["AJAX_POST"] == "Y"):?>
                                    <a class="color-theme-hover" href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToHide"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_HIDE")?>">
                                <?else:?>
                                    <a class="color-theme-hover" href="<?=$comment["urlToHide"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>" title="<?=GetMessage("BPC_MES_HIDE")?>">
                                <?endif;?>
                        ...
                        if(strlen($comment["urlToDelete"])>0)
                        {
                            ...
                            <span class="blog-comment-delete">
                                <?if($arParams["AJAX_POST"] == "Y"):?>
                                    <a class="color-theme-hover" href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) deleteComment('<?=$comment["urlToDelete"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_DELETE")?>">
                                <?else:?>
                                    <a class="color-theme-hover" href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>'" title="<?=GetMessage("BPC_MES_DELETE")?>">
                                <?endif;?>
