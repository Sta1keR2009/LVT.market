...
while ($comment = $dbComment->Fetch()) {
	if ($comment['UF_ASPRO_COM_RATING']) {
		...
		if (
			in_array('RATING', $arParams['REVIEW_FILTER_BUTTONS']) 
			&& !in_array($comment['UF_ASPRO_COM_RATING'], $arParams['AVAILABLE_RATING'])
		) {
			$arParams['AVAILABLE_RATING'][] = $comment['UF_ASPRO_COM_RATING'];
		}
	}
...
$arParams['SORT_PROP'] = $_SESSION['REVIEW_SORT_PROP'] ?: 'UF_ASPRO_COM_RATING';
$arParams['SORT_ORDER'] = $_SESSION['REVIEW_SORT_ORDER'] ?: 'desc';
...
if ($arResult["PagesComment"]) {
	...
	foreach ($arResult["PagesComment"] as $page => &$comments) {
		foreach ($comments as &$comment) {
			...
			if (
				\Aspro\Lite\Template\Filter\Review::checkRatingFilter(
					$arParams['REVIEW_FILTER_BUTTONS'], 
					$arComments[$comment['ID']]['UF_ASPRO_COM_RATING']
				) 
				|| \Aspro\Lite\Template\Filter\Review::checkTextFilter(
					$arParams['REVIEW_FILTER_BUTTONS'], 
					$comment['HAS_COMMENT']
				)
				|| \Aspro\Lite\Template\Filter\Review::checkPhotoFilter(
					$arParams['REVIEW_FILTER_BUTTONS'], 
					isset($comment['IMAGES'])
				)
			) {
				continue;
			}
	...
	if ($arCommentsSort) {
		...
		if ($arParams['SORT_PROP'] == 'DateFormated') {
			...
			$arParams['SORT_ORDER'] = $arParams['SORT_ORDER'] == 'desc' ? SORT_DESC : SORT_ASC;
			\Bitrix\Main\Type\Collection::sortByColumn($arCommentsSort, array($arParams['SORT_PROP'] => array($arParams['SORT_ORDER']), 'HAS_COMMENT' => array(SORT_DESC), "DATE_CREATE" => array(SORT_DESC)));
		}
