                                            ...
                                            <div class="form-group">
                                                <div class="input">
                                                    ...
                                                    <div class="add_file color-theme">
                                                        <span class="font_12"><?=GetMessage('JS_FILE_ADD');?></span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                ...
                <? if ($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview"): ?>
                        ...
                        <div class="blog-comment-cont-white">
                            <div class="blog-comment-info">
                                ...
                                <div class="left_info">
                                    ...
                                    <div class="blog-comment__date color_999 font_14"><?=FormatDate('d F Y, H:i', MakeTimeStamp($comment["DateFormated"]))?></div>
                                </div>
                                ...
                                <div class="blog-comment-post__item blog-comment-meta">
                                    ...
                                    <? if (strlen($comment["urlToDelete"])>0): ?>
                                        <span class="blog-comment-delete blog-comment-action color_222">
                                            <?if($arParams["AJAX_POST"] == "Y"):?>
                                                <a href="javascript:void(0)" class="blog-comment-action__link dotted dark_link font_14" onclick="if(confirm('<?=GetMessage('BPC_MES_DELETE_POST_CONFIRM')?>')) deleteComment('<?=$comment['urlToDelete'].'&'.bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($request['IBLOCK_ID'])?>&ELEMENT_ID=<?=htmlspecialcharsbx($request['ELEMENT_ID'])?>', '<?=$comment['ID']?>');" title="<?=GetMessage("BPC_MES_DELETE")?>">
                                            <?else:?>
                                                <a href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($request["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($request["ELEMENT_ID"])?>'" class="blog-comment-action__link dotted dark_link font_14" title="<?=GetMessage("BPC_MES_DELETE")?>">
                                            <?endif;?>
                    ...
                    if (
                        ...
                        $commentPreview = array(
                            ...
                            "TextFormated" => htmlspecialcharsbx($post["commentFormated"]),
