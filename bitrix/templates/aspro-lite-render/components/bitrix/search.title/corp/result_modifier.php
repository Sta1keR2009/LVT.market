...
use Bitrix\Currency\CurrencyTable,
	Aspro\Lite\SearchQuery,
	CLite as Solution,
	CLiteCache as SolutionCache,
	Aspro\Lite\SearchTitle;
...
if (
	...
	&& ($_REQUEST[GetchipsPartnumberSearchHelper::MODE_PARAM] ?? '') === GetchipsPartnumberSearchHelper::MODE_VALUE
) {
	$arResult['CATEGORIES'] = [];
...
$arCatalogs = $arOffersIbloks = array();
if (TSolution\Product\Price::$catalogInclude) 
{
	...
	{
		$iblockId = $ar["PRODUCT_IBLOCK_ID"] ?: $ar["IBLOCK_ID"];
		$arCatalogs[$iblockId] = 1;

		if ($ar["OFFERS_IBLOCK_ID"]) {
			$arOffersIbloks[$ar["OFFERS_IBLOCK_ID"]] = 1;
		}
	}
}
...
if (CModule::IncludeModule("iblock")) {
	if (!empty($arResult["ELEMENTS"])){
		if (TSolution\Product\Price::$catalogInclude) {
			...
			$bHideNotAvailable = TSolution::GetFrontParametrValue('HIDE_NOT_AVAILABLE') === 'Y';
			$bHideNotAvailableOffers = TSolution::GetFrontParametrValue('HIDE_NOT_AVAILABLE_OFFERS') === 'Y';
		...
		if (!TSolution\Product\Price::$catalogInclude) {
			...
			$arSelect = array_merge($arSelect, $arPriceProps);
		}
		...
		if(TSolution\Product\Price::$catalogInclude)
		{
			if (
				...
				if($arRegion){
					...
					if($arStores)
					{
						if(TSolution::checkVersionModule('18.6.200', 'iblock')){
							$arAmountFilter["LOGIC"] = "OR";
							$arAmountFilter[] = array('TYPE' => array('2','3')); //complects and offers
							$arAmountFilter[] = array(
							...
							if(count($arStores) > 1){
								$arAmountFilter = array('LOGIC' => 'OR');
								foreach($arStores as $storeID)
								{
									$arAmountFilter[] = array(">CATALOG_STORE_AMOUNT_".$storeID => 0);
								}
							}
							else{
								foreach($arStores as $storeID)
				...
				$arTmpOffersFilter = array();
				if ($bHideNotAvailableOffers) {
					$arTmpOffersFilter['CATALOG_AVAILABLE'] = 'Y';
					$arTmpOffersFilter[] = $arAmountFilter;
				}

				$arTmpCatalogFilter = array();
				if ($bHideNotAvailable) {
					$arTmpCatalogFilter['CATALOG_AVAILABLE'] = 'Y';
					$arTmpCatalogFilter[] = $arAmountFilter;
				}

				$arFilter[] = [
					'LOGIC' => 'OR',
					[
						'IBLOCK_ID' => array_keys($arOffersIbloks),
					] + $arTmpOffersFilter,
					[
						'!IBLOCK_ID' => array_keys($arOffersIbloks),
					] + $arTmpCatalogFilter,
		...
		while($arElement = $rsElements->Fetch())
		{
			...
			if($GLOBALS['arTheme']['USE_REGIONALITY']['VALUE'] === 'Y' && $GLOBALS['arTheme']['USE_REGIONALITY']['DEPENDENT_PARAMS']['REGIONALITY_FILTER_ITEM']['VALUE'] === 'Y'){
				if($arSectionsIds_NotInRegion = TSolution::getSectionsIds_NotInRegion($arElement["IBLOCK_ID"])){
			...
			if ($arOffersIbloks[$arElement['IBLOCK_ID']]) {
				if ($bHideNotAvailableOffers) {
			...
			if (TSolution\Product\Price::$catalogInclude) {
				...
				$arElement["PRICE"]["VALUE"] = $arElement["PROPERTY_PRICE_VALUE"];
				$arElement["PRICEOLD"]["VALUE"] = $arElement["PROPERTY_PRICEOLD_VALUE"];
				$arElement["ECONOMY"]["VALUE"] = $arElement["PROPERTY_ECONOMY_VALUE"];
				$arElement["PRICE_CURRENCY"]["VALUE"] = $arElement["PROPERTY_PRICE_CURRENCY_VALUE"];
				$arElement["FILTER_PRICE"]["VALUE"] = $arElement["PROPERTY_FILTER_PRICE_VALUE"];
			}	
			...
			if($arOffers){
				...
				$arResult["ELEMENTS"][$arElement["ID"]]["MIN_PRICE"]=TSolution::getMinPriceFromOffersExt(
						$arOffers,
...
foreach($arResult["SEARCH"] as $i => &$arItem)
{
	...
	{
		case "iblock":
			if(array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"]) && is_array($arResult["ELEMENTS"][$arItem["ITEM_ID"]])){
