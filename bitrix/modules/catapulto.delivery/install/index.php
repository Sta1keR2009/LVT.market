...
class catapulto_delivery extends CModule
{
    ...
    protected function getDB()
    {
        return [
                ...
                'catapulto_delivery_warehouses_free_delivery' => 'WarehousesFreeDelivery',
            ...
            if (!$DB->Query("SELECT 'x' FROM " . $name, true)) {
                ...
                if ($this->errors !== false) {
                    ...
                    if ($name == 'catapulto_delivery_warehouses') {
                        //Создание 1-го склада из старых настроек сделано через updater
                    }
                }
            }
        }
    ...
    {
        //Обязательное поле для ввода ключа Яндекс карт для новых установок модуля
        \Bitrix\Main\Config\Option::set($this->MODULE_ID, 'system_widget_yandex_key_required', 'Y');
    ...
    private function ShowDataSaveForm()
    {
        ...
        <form action="<?= $APPLICATION->GetCurPage() ?>" method="get">
            ...
            <p><input type="checkbox" name="CATAPULTO_DELIVERY_savedata" id="CATAPULTO_DELIVERY_savedata" value="Y" checked="checked"/><label for="savedata"><?
                    echo GetMessage('MOD_UNINST_SAVE_TABLES') ?></label><br></p>
