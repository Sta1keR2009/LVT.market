(function () {
  ...
  function renderMarkdown(text) {
    ...
    // Product cards: [card](url|img_url|title) — MUST be before link regex
    html = html.replace(/\[card\]\(([^|]+)\|([^|]+)\|([^)]+)\)/g, function(m, url, img, title) {
      var card = '<a href="' + url + '" target="_blank" class="lvt-ai-product-card">';
      if (img) card += '<img src="' + img + '" class="lvt-ai-product-card__img" loading="lazy" />';
      card += '<span class="lvt-ai-product-card__title">' + title + '</span></a>';
      return card;
    });
    // Links: [text](url)
    ...
    // Images: ![alt](url)
    html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="lvt-ai-msg__img" loading="lazy" />');
    // Line breaks
    html = html.replace(/\n/g, "<br>");
    return html;
  }
